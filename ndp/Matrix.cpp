...
void Matrix::removeRelation(std::string vertexA, std::string vertexB) {
    ...
    rows[posB].reset(posA);
}

int Matrix::grow() {
    std::string *novosLabels = new std::string[size + 1];
    Row *novasLinhas = (Row *) malloc(sizeof(Row) * (size + 1));
    rows->columns = new int[1];
    rows->columns[0] = 0;

    int i = 0;
    for (; i < size; i++) {
        novosLabels[i] = labels[i];
        novasLinhas[i] = rows[i];
        novasLinhas[i].add();
    }
    labels = novosLabels;
    rows = novasLinhas;
